...
Batch processing for mood trends, insights, and user analytics
Separated from maintenance tasks for better organization
"""
...
from app.services.redis_service import redis_service, redis_analytics_service
...
def analyze_entry_length_correlation(entries: List) -> Dict[str, Any]:
    ...
    try:
        ...
        for entry in entries:
            if entry.mood and entry.content:
        ...
        for mood, lengths in mood_lengths.items():
            mood_avg_lengths[mood] = np.mean(lengths) if lengths else 0
        
        # Determine correlation strength
        if len(mood_avg_lengths) >= 2:
            values = list(mood_avg_lengths.values())
            correlation_strength = np.std(values) / np.mean(values) if np.mean(values) > 0 else 0
        else:
            correlation_strength = 0
        
        # Generate insight
        if correlation_strength > 0.3:
            longest_mood = max(mood_avg_lengths, key=mood_avg_lengths.get)
            shortest_mood = min(mood_avg_lengths, key=mood_avg_lengths.get)
...
def analyze_topic_mood_correlation(entries: List) -> Dict[str, Any]:
    ...
    try:
        ...
        for entry in entries:
            if entry.topic_id and entry.mood:
...
def generate_correlation_recommendations(correlations: Dict[str, Any]) -> List[str]:
    ...
    return recommendations[:6]# backend/app/tasks/analytics.py
