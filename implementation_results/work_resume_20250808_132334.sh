...
echo "⏰ Claude quota will reset at: 2025-08-08 15:00:00"
echo "🕐 Current time: $(date)"
echo ""

# Calculate remaining wait time dynamically (in case script is run later)
reset_timestamp=$(date -d "2025-08-08 15:00:00" +%s 2>/dev/null)
...
if [[ $wait_seconds -gt 0 ]]; then
    ...
    echo "🎯 Will auto-resume at: 2025-08-08 15:00:00"
    echo ""
    echo "⏱️  Countdown:"
    ...
    while [[ $wait_seconds -gt 0 ]]; do
        ...
        if [[ $hours -gt 0 ]]; then
            printf "\r   🕐 %02d:%02d:%02d remaining - Resume at 2025-08-08 15:00:00" $hours $minutes $seconds
        else
            printf "\r   ⏰ %02d:%02d remaining - Resume at 2025-08-08 15:00:00" $minutes $seconds
        fi
