...
# Test the final corrected Claude CLI parser
echo "Testing Claude CLI JSON parser..."

# Simulate Claude CLI result message with tool usage
...
echo "Expected: Should show tool usage details"
echo "Result:"

# Extract the parse function and test it
parse_claude_json() {
    local line="$1"
    if [[ -n "$line" ]]; then
        python3 -c "
...
echo ""
echo "If you see tool usage details above, the parser is working correctly!"
